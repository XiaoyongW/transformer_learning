...
def build_vocab(tokenized_sentences):
    vocab = Counter() # 用于统计各个分词出现的频率：实际上是为了不重复的记录每个分词；
    ...
    vocab['<eos>'] = 2 # 最终标识符；
    print('--build vocab--')
    return vocab # 构建词表：每个分词对应一个唯一数字；
...
def numericalize(tokens, vocab):
    ...
    result = pad_sequence([torch.LongTensor(np.array(sentence)) for sentence in result],
                        batch_first=True, 
                        padding_value=vocab['<pad>']) # 使用padding;
    return result
...
class TranslationDataset(Dataset):
    def __init__(self, src_numerical, tgt, tgt_numerical, src_vocab, tgt_vocab,pad=0):
        ...
        self.src_mask = (src_numerical != pad).unsqueeze(-2) # 生成原输入序列的掩码；
    ...
    def __getitem__(self, idx): # 获取源文本和目标文本；
        ...
        return { # 返回：原序列；原序列掩码；目标序列；目标序列掩码；预测目标序列；真值；
            ...
            'src_mask':Variable(self.src_mask[idx]),
            'tgt': self.tgt[idx].type(torch.long),
            'tgt_mask': self.tgt_mask[idx],
            'tgt_y': self.tgt_y[idx].type(torch.long),
